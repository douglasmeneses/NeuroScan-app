...
router.get("/stats", async (req: any, res: any) => {
  try {
    ...
    const [totalUsuarios, totalQuestionarios, totalRespostas] =
      await Promise.all([
...
router.get("/tempo-questionarios", async (req: any, res: any) => {
  try {
    ...
    const perguntasIds = temposPorQuestionario.map((t: any) => t.pergunta_id);
    ...
    const resultado = temposPorQuestionario.map((tempo: any) => {
      const pergunta = perguntas.find((p: any) => p.id === tempo.pergunta_id);
      return {
...
router.get("/usuarios-stats", async (req: any, res: any) => {
  try {
    ...
    const resultado = usuariosStats.map((usuario: any) => {
      const totalCliques = usuario.respostas.reduce(
        (sum: number, r: any) => sum + r.quantidade_cliques,
        0,
      );
      const totalPassos = usuario.respostas.reduce(
        (sum: number, r: any) => sum + r.quantidade_passos,
        0,
      );
      const tempoTotal = usuario.respostas.reduce(
        (sum: number, r: any) => sum + r.duracao,
        0,
      );
      const tempoIdleTotal = usuario.respostas.reduce(
        (sum: number, r: any) => sum + r.idle,
        0,
      );
      ...
      return {
        ...
        tempo_medio_por_resposta:
          usuario.respostas.length > 0
            ? tempoTotal / usuario.respostas.length
            : 0,
      };
    });
